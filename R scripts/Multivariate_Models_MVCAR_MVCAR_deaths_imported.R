...
# using the MCAR prior for the spatial random effects and the MCAR prior for   #
...
Rg = structure(.Data = c(1, 0, 0, 1), .Dim = c(2,2))
Rg
...
#constants
LSTConsts <-list(T, m, adj, sumNumNeigh, num, L, e, R, Rg, L_g, adj_g, num_g, weights_g)
names(LSTConsts) <-c("T","m","adj","sumNumNeigh","num", "L", "e", "R", "Rg", "L_g",
  ...
  STCode_model <-nimbleCode(
    {
      for (i in 1:m)
      {
        ...
        { y_d[i,k]~dpois(mu_d[i,k])
          ...
          log(theta_d[i,k])<-a0_d+phig[k, 1]+phi[i,1]+v_d[i]+psi_d[i,k]
          ...
          log(theta_ic[i,k])<-a0_ic+phig[k, 2]+phi[i,2]+v_ic[i]+psi_ic[i,k]
          psi_ic[i,k]~dnorm(0,taupsi_ic)
          Dp_ic[i,k]<-(y_ic[i,k]- pred_y_ic[i,k])**2
          
          exp_g[k, 1] <- exp(phig[k, 1])
          exp_g[k, 2] <- exp(phig[k, 2])
      ...
      for (k in 1:2)
      {
        g[1:T,k]~dcar_normal(adj_g[1:L_g],weights_g[1:L_g],num_g[1:T],taug[k],zero_mean=1)	
      }
      
      Precg[1:2,1:2]~dwish(Rg[1:2,1:2],2)
      Covg[1:2,1:2]<-inverse(Precg[1:2,1:2])
      Acholg[1:2,1:2]<-chol(Covg[1:2,1:2])
      
      for(i in 1:T) {
        phig[i,1:2]<-Acholg[1:2,1:2]%*%g[i,1:2]
      }
      ...
      taug[1]<-1/pow(sdg_d,2)
      sdg_d~dunif(0,10)
      taug[2]<-1/pow(sdg_ic,2)
      sdg_ic~dunif(0,10)
      ...
      sigg[1]<-sqrt(Covg[1,1])
      sigg[2]<-sqrt(Covg[2,2])
      
      cor12g<-Covg[1,2]/(sigg[1]*sigg[2])   
...
LSTinits_model <-list(a0_d=-12,
                           sdv_d=0.1,
                           sdu_d=0.1, sdg_d=0.1, sdpsi_d=0.1,
                           ...
                           a0_ic=-12,
                           sdv_ic=0.1,
                           sdpsi_ic=0.1, sdu_ic = 0.1, sdg_ic = 0.1,
                           ...
                           Precg=structure(.Data=c(1, 0,0,1),.Dim=c(2,2)))
...
output_model <- parLapply(cl = cluster1, 
                               ...
                               prms = params_model)
output_model
MCMCsummary(output_model, params = c("cor12g", "cor12", "sigg", "Covg",
                                          "Cov", "sig", "mspe_d", "mspe_ic"))
...
ggplot(temporal_means_df, aes(x=weeks, group=1)) + 
  ...
  labs(x = "Week", y = "exp(g)", title = "Overall temporal trend of deaths (lag 4)")
